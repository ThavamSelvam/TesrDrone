...
class InnerMenuGridAdapter(val context: Context?, val data: List<ResponseData>,val click:OnClickEvent) :
    RecyclerView.Adapter<InnerMenuGridAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.cons.setOnClickListener {
            data.get(position).title?.let { click.onVieClick(data.get(position).title!!) }
        }
    }
